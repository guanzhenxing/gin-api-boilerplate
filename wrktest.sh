...
使用方式 ( 测试 API 性能)：
1. 启动 gin-api-boilerplate (8080端口)
2. 执行测试脚本: ./wrktest.sh
...
1. 执行命令： ./wrktest.sh diff gin-api-boilerplate1_wrk.dat http_wrk.dat
 
> Note: 需要确保系统安装了 wrk 和 gnuplot 工具
EOF

t1="gin-api-boilerplate2" # 对比图中红色线条名称
t2="http" # 对比图中粉色线条名称
jobname="gin-api-boilerplate2" # 本次测试名称
