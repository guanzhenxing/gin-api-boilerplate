...
func Logging() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if path == "/actuator/health" || path == "/actuator/ram" || path == "/actuator/cpu" || path == "/actuator/disk" {
			return
		}
		...
		if err := json.Unmarshal(blw.body.Bytes(), &response); err != nil {
			log.Error("response body can not unmarshal to model.Response struct, body: "+blw.body.String(), zap.Error(err))
			code = ecode.InternalServerError.Code()
			message = err.Error()
		} else {
			code = response.Code
